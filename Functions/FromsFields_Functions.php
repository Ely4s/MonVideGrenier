...
function check_IfLike_Id($id, $errorOn = true)
{
    ...
    {
        if($errorOn) if($errorOn) $_SESSION['errors']['Id_NotLike_Id'] = "Identifiant incorrect";
...
function check_IfValid_Username(&$username, $isNew, $errorOn = true)
{
    ...
    //Check if username is valid for signup
    if(empty($username))
    {
...
{
    //Check if phoneNumber is valid for signup
...
function check_IfValid_City(&$city, $errorOn = true)
{
    ...
    //Check if city is valid for signup
    if(empty($city))
    {
...
{
    //Check if password and passwordConf are valid for signup
...
function check_IfStrictEqual($value1, $value2)
{
