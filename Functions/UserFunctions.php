...
function User_New(&$username, &$email, &$email_privacy, &$phoneNumber, &$phoneNumber_privacy, &$city, &$city_privacy, &$password)
{
    ...
    $stmt->bind_param('ssisisiss', $username, $email, $email_privacy, $phoneNumber, $phoneNumber_privacy, $city, $city_privacy, $password_hashed, $datetime);
...
function User_Connect($username, $password, $isNewAccount = false)
{
    ...
    if($stmt->execute())
    {
        ...
        if(password_verify($password, $user['USERS_password']))
        {
            ...
            $_SESSION['emailPrivacy'] = $user['USERS_emailPrivacy'];
            $_SESSION['phoneNumberPrivacy'] = $user['USERS_phoneNumberPrivacy'];
            $_SESSION['cityPrivacy'] = $user['USERS_cityPrivacy'];
            $_SESSION['profilPicture'] = $user['USERS_profilPicturePath'];
...
/* Mise à jour de la confidentialité l'email
...
/* Mise à jour de la confidentialité du numéro de téléphone
...
/* Mise à jour du numéro de la ville
...
/* Mise à jour de la confidentialité de la ville
...
/* Mise à jour photo de profil
    ...
    et on crée la nouvelle après redimension et fix de l'orientation
